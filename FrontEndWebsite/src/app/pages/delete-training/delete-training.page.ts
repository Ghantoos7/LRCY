...
export class DeleteTrainingPage implements OnInit {
  ...
  deleteTrainingForUsers() {
    this.adminService.deleteTrainingForUsers(this.selectedTrainings, this.selectedUsers).subscribe((response: any) => {
      const parsedResponse = JSON.parse(JSON.stringify(response));
      if(parsedResponse.status == 'success') {
        this.alertController.create({
          header: 'Success',
          message: 'Trainings removed successfully!',
          buttons: ['OK']
